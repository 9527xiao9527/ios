...
class UserInfo {
  constructor(_0x9880) {
    ...
    'Cookie': this.token,
    'f-pTraceId': 'WVNet_WV_5-5-130',
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8',
    'Connection': 'keep-alive',
    'Host': 'developer.aliyun.com',
    'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 AliApp(Aliyun/7.6.1) WindVane/8.7.2 1284x2778 WK',
    'f-refer': 'wv_h5',
  ...
  async getArticles() {
    try {
      ...
      //console.log("这是响应" + _0x3c4654)
      const _0x4d1f6a = $.Cheerio.load(_0x3c4654);
      //console.log(_0x4d1f6a)
    ...
    try {
      const _0x40283e = {
       ...
       // _0x3273a8 = await _0x3273a8.text(); 
        ...
        // 使用正则表达式匹配 JSON 数据
        const jsonRegex = /\{.*\}/s; 
        const match = _0x3273a8.match(jsonRegex);

        if (!match) {
            throw new Error("未找到有效的 JSON 数据");
        }

        const _0x540a75 = match[0];
        const _0x4cf1fd = JSON.parse(_0x540a75);

        if (!_0x4cf1fd.data || !_0x4cf1fd.data.uccCsrfToken) {
            throw new Error("未找到 uccCsrfToken 字段");
        }
...
async function loadCheerio() {
  ...
  return new Promise(async _0x8b6ab8 => {
    $.getScript("https://raw.githubusercontent.com/Yuheng0101/X/main/Utils/cheerio.js").then(_0x61da04 => {
  ...
  return new class {
    ...
    loaddata() {
      ...
      }
      {
        ...
        const r = !s && this.fs.existsSync(e);
        if (!s && !r) {
          return {};
        }
        {
    ...
    lodash_get(t, e, s = undefined) {
      ...
      let a = t;
      for (const t of r) if (a = Object(a)[t], undefined === a) {
        return s;
      }
      return a;
    }
    ...
    setdata(t, e) {
      ...
      if (/^@/.test(e)) {
        const [, r, a] = /^@(.*?)\.(.*?)$/.exec(e);
        const i = this.getval(r);
        const o = r ? "null" === i ? null : i || "{}" : "{}";
        try {
      ...
      switch (t.body && t.headers && !t.headers["Content-Type"] && !t.headers["content-type"] && (t.headers["content-type"] = "application/x-www-form-urlencoded"), t.headers && (delete t.headers["Content-Length"], delete t.headers["content-length"]), undefined === t.followRedirect || t.followRedirect || ((this.isSurge() || this.isLoon()) && (t["auto-redirect"] = false), this.isQuanX() && (t.opts ? t.opts.redirection = false : t.opts = {
        ...
        case "Node.js":
          ...
          this.got[s](a, i).then(t => {
            ...
            const n = r.decode(o, this.encoding);
            e(null, {
              status: s,
              statusCode: a,
              headers: i,
